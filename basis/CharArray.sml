 ...
 *
 * Note: Array type kept transparent to allow trivial implementation
 *       of CharArraySlice.
 *)
...
struct
    ...
    fun copyVec{src, dst, di}	= if di < 0
				  orelse length dst < di + CharVector.length src
				  then raise Subscript
				  else copyVec'(src, dst, di, 0)
    and copyVec'(src,dst,di, i)	= if i = CharVector.length src then () else
