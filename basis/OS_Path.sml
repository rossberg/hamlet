...
struct
	...
	fun splitDirFile s =
		...
		case splitLast arcs
		  of NONE =>
		     {dir  = toString{isAbs=isAbs, vol=vol, arcs=nil},
		      file = ""}
		   | SOME(arcs, arc) =>
		     {dir  = toString{isAbs=isAbs, vol=vol, arcs=arcs},
		      file = arc}
	    end
	...
	fun splitBaseExt s =
		...
		case splitLast arcs
		  of NONE =>
		     {base = toString{isAbs=isAbs, vol=vol, arcs=nil},
		      ext = NONE}
		   | SOME(arcs, arc) =>
	...
	fun concat(s1, s2) =
		...
		if vol2 = "" orelse vol1 = vol2
	        then toString{isAbs=isAbs, vol=vol1, arcs=concatArcs(arcs1, arcs2)}
		else raise Path
