...
struct
    ...
    fun fromList x		= (use{b="Vector.fromList"} :
							'a list-> 'a vector) x

    fun tabulate(n, f)		= fromList(List.tabulate(n, f))

    fun update(vec, i, x)	= tabulate(length vec, fn i' =>
					   if i = i' then x else sub(vec,i'))
    ...
    and collate'(f,vec1,vec2,i)	= case (i = length vec1, i = length vec2)
				    of (true,  true ) => EQUAL
				     | (true,  false) => LESS
				     | (false, true ) => GREATER
				     | (false, false) =>
				  case f(sub(vec1, i), sub(vec2, i))
				    of EQUAL => collate'(f, vec1, vec2, i+1)
				     | other => other

    fun concat l		= fromList(List.concat
					    (List.map (foldr op:: []) l))
end
