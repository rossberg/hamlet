...
struct
    ...
    fun mapPartial f   nil	= nil
      | mapPartial f (x::l')	= case f x of NONE   => mapPartial f l'
    ...
    fun filter f   nil		= nil
      | filter f (x::l')	= if f x then x :: filter f l'
					 else filter f l'

    fun partition f l			= partition'(f, l, nil, nil)
    and partition'(f,  nil,  pos, neg)	= (rev pos, rev neg)
      | partition'(f, x::l', pos, neg)	= if f x
					  then partition'(f, l', x::pos, neg)
					  else partition'(f, l', pos, x::neg)
    ...
    fun tabulate(n, f)		= if n < 0 then raise Size else
				  tabulate'(n, f, 0, nil)
    ...
    fun collate f (  nil,     nil  )	= EQUAL
      ...
      | collate f (x1::l1', x2::l2')	= case f(x1, x2)
