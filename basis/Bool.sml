...
struct
    ...
    fun StringCvt_dropl p f src	= case f src
 				    of NONE => src
				     | SOME (c, src') =>
				       if p c then StringCvt_dropl p f src'
					      else src
    fun StringCvt_skipWS f s	= StringCvt_dropl Char_isSpace f s

    fun StringCvt_scanString f s = case f (reader s) 0
				    of NONE => NONE
				     | SOME (a, _) => SOME a
    and reader s i		= SOME (String_sub(s, i), i+1)
